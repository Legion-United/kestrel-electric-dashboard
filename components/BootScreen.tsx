...
export default function BootScreen() {
  ...
  React.useEffect(() => {
    let booted = false;
    try { booted = sessionStorage.getItem("kestrel-booted") === "1"; } catch {}
    if (booted) return;
    setShow(true);
    const r = requestAnimationFrame(() => setGo(true));
    const t1 = setTimeout(() => setOut(true), 1150);
    const t2 = setTimeout(() => {
      try { sessionStorage.setItem("kestrel-booted", "1"); } catch {}
      setShow(false);
    }, 1600);
  ...
  return (
    <div className={`boot ${out ? "out" : ""}`}>
      ...
      <div className="boot-inner boot-quick">
        ...
        <div className="boot-word"><b>KESTREL</b><span>Solo Operator OS</span></div>
